...
def print_final_dec_time():
    # 确保收集到了足够的数据（排除只有几帧的异常情况）
    if len(global_dec_times) > 50:
        valid_times = global_dec_times[50:] # 同样去掉前 50 帧的 Warm-up 噪音
        avg_time = sum(valid_times) / len(valid_times)
        print(f"\n{'='*50}")
        print(f" =====> [Final Result] Average Dec.Time: {avg_time:.2f} ms <=====")
        print(f"{'='*50}\n")

# 注册该函数，使其在 Python 脚本运行结束时自动执行
atexit.register(print_final_dec_time)
# ====================添加导入和全局测速逻辑结束====================
...
class OpenHead(StreamPETRHead):
    ...
    def forward(self, outs_roi, img_metas, topk_indexes=None, **data):
        ...
        # --- 添加计时结束 ---
        torch.cuda.synchronize()
        dec_time = (time.time() - start_dec) * 1000 # 转换为 ms
        global_dec_times.append(dec_time)
        # --- 添加计时结束 ---
